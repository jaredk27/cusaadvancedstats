...
defensive_stats <- cusa_advanced_stats %>% 
...
defensive_stats <- rounded_stats(defensive_stats, 2)

# Now let's rename UTSA's team name to avoid UTSA Twitter's ire

defensive_stats[13, 2] = "UTSA"

# Draw the table and format it

draw_table <- function() {
  defensive_stats %>% gt() %>%
    tab_header(title = "Conference USA Advanced Stats - Defense", subtitle="Through Week 11 of the 2021 season") %>%
    ...
    cols_label(
      ...
      team="Team",
      def_ppa="PPA",
      def_success_rate="Success Rate",
      def_explosiveness="Explosiveness",
      def_power_success="Power Success",
      def_stuff_rate="Stuff Rate",
      def_line_yds="Line Yards",
      def_pts_per_opp="Stopping Drives",
      def_havoc_total="Havoc"
    ) %>%
    data_color(
      columns = c(def_ppa),
